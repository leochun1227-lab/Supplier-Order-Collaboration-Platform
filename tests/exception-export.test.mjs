...
const orders=[{id:'order-a',po:'00001234',item:'0010',so:'SO-1',sapPart:'SAP-OLD',soPart:'NEW-PART',unit:'EA',soUnit:'PCS',sapAsOf:'2026-09-10',importEvidence:[{file:'original.xlsx',sheet:'Sheet1',row:4}]}]
...
test('exception export includes both material originals and provenance without mutating records',()=>{
  ...
  assert.equal(record.PO,'00001234');assert.equal(record['订单行'],'0010')
...
test('real XLSX has all result rows, frozen filter headers, and literal safe text cells',()=>{
  ...
  assert.match(sheet,/<autoFilter ref="A1:T84"\/>/)
