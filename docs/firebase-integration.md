...
`src/runtime-config.mjs` 明确启用公开测试；页面启动后自动从 Firebase 加载真实工作区。`src/public-firebase-store.mjs` 使用条件写入保存追加版本，15 秒检查其他操作产生的新版本；保存失败显示错误、恢复本地记录，版本争用拒绝覆盖。公开规则下任何人仍可以从应用之外改写数据，所以这不是身份认证或防篡改的审计保证。操作人标记为未验证的公开测试用户。

默认未完成口径是台账剩余待发（初始化 99 条），全部视图包含 1,253 条，取消 15 条。1,139 个 OK 标记不作为收货事实。1,140 条累计报发汇总不假定为逐批物流事件。国内 ETA 不复制成澳洲 ETA；用途缺少明确字段时待确认，采购员不自动推断用途。未核实单位或关联的记录不参与数量求和，价格未确认则不进入货值。

已实测公开测试工作区的保存、重载、清空字段及并发冲突拒绝，测试工作区已删除。SAP 未修改，定时 SAP 同步仍未启用。下面的内部登录方案、私有规则和管理端脚本保留供之后切换正式权限时使用；它们不是当前公开测试的运行路径。
