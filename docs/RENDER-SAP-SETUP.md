...
**网页已读取 `sap/current` 中的 PO 表，刷新 PO／行、料号、单位及价格参考，并重新计算对应异常。** 每 15 秒检查新指针，关联及顺序编号规则见 [SAP 与总表比对](sap-workbook-comparison.md)。它不会重建现有台账；收货、在途、SO、冲销和分批数量仍需经过核实的映射后接入。平台维护的 ETA、延期、报发、快递号、集装箱号及历史版本不会被本脚本覆盖。
