...
function saveShipment(input){
  ...
  try{updateShipment(s,input,stamp());for(const a of allocations(s))addEvent(orders.value.find(o=>o.id===a.order),`更新批次 ${s.id}：${input.note}`,`Updated batch ${s.id}: ${input.note}`);checks.value=reconcile(orders.value,shipments.value,checks.value);toast(label('物流与延期信息已保存，所有关联订单已更新。','Logistics and delay saved. All linked orders updated.'))}catch(e){throw new Error(tr(actionErrors[e.message]||e.message))}
}
...
function saveCollaborationDates(o,input){
  ...
  const old={etd:o.promisedEtd,china:o.chinaEta,delayStatus:o.dispatchDelayStatus||'unknown',delayReason:o.dispatchDelayReason||'',plan:o.remainingPlan||''}
  ...
  o.dispatchUpdates=[{at:stamp(),before:old,after:{etd:o.promisedEtd,china:o.chinaEta,delayStatus:o.dispatchDelayStatus,delayReason:o.dispatchDelayReason,plan:o.remainingPlan},note:input.reason},...(o.dispatchUpdates||[])]
...
<template>
  <div class="workspace" :class="{'overview-layout':page==='overview'}">
    ...
    <ShipmentEditor :model-value="modalOpen && modal==='shipment'" :shipment="activeShipment" :perform="saveShipment" @update:model-value="modalOpen=$event"/>
