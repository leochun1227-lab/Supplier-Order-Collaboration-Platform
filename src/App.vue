...
function applyRemote(packet){cloudGeneration=packet.generation;applyState(packet.state);cloudRevision.value=packet.revision;cloudSavedAt.value=packet.savedAt;cloudEmail.value=packet.email}
...
async function savedAction(name,action){
  ...
  if(!cloudConnected.value)return action()
  ...
  <div class="workspace" :class="{'overview-layout':page==='overview'}">
    ...
    <div class="main-shell">
      ...
      <main :inert="cloudBusy" :class="{'overview-main':page==='overview','orders-main':page==='orders'}">
        ...
        <CloudPanel v-if="page==='data'" :configured="cloudConfigured" :connected="cloudConnected" :busy="cloudBusy" :status="cloudStatus" :error="cloudError" :email="cloudEmail" :revision="cloudRevision" :saved-at="cloudSavedAt" :deleted="shipments.filter(s=>s.deletedAt)" @connect="loginCloud" @disconnect="leaveCloud" @restore="restoreShipment"/>
    ...
    <Dialog v-if="modal!=='shipment'" v-model="modalOpen" :options="{title:tr(modalTitle),size:modal==='help'?'xl':'lg'}"><template #body-content>
      ...
      <template v-else-if="modal==='help'"><div class="help-content"><span class="demo-chip">{{tr("Frappe UI · 交互原型")}}</span><p>{{tr(label('这是一份使用 Frappe UI 组件的功能预览。Firebase 内部试用和 SAP 同步的启用情况请查看数据中心。','This preview uses Frappe UI components. Check Data center for Firebase internal pilot and SAP sync activation status.'))}}</p><ol><li>{{tr("切换至")}} <b>{{tr("Longtree · 供应商")}}</b>{{tr("，打开待确认订单，提交数量和交付批次。")}}</li><li>{{tr("对已确认、未发运的订单提交交期变更。")}}</li><li>{{tr("切回")}} <b>{{tr("Regent · 内部采购")}}</b>{{tr("，在“交期与承诺”接受或退回变更。")}}</li><li>{{tr("在“异常与待办”记录处理结果，并查看更新后的报表。")}}</li></ol><p>{{tr(label('所有数据为虚构样本。数据中心显示当前保存状态；仅在成功连接云端后，操作才会持久保存。','All data is fictional. Data center shows save status; changes persist only after a successful cloud connection.'))}}</p><a href="https://github.com/frappe/frappe-ui" target="_blank" rel="noreferrer">{{tr("查看 Frappe UI 开源项目 ↗")}}</a></div></template>
