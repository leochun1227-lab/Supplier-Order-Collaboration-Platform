...
import { workspaceOrders, workspaceShipments, reconcile, updateCase, resolveFact, recordDispatch, applyDemoSnapshot, validDate, label, reportedQty, allocations } from './reconciliation.mjs'
...
const checks=ref(reconcile(orders.value,shipments.value)), synced=ref(false),reviewOpen=ref(false),reviewCase=ref(null),dispatchOpen=ref(false),dispatchSelected=ref(null)
const allIssues=computed(()=>[...issues.value,...checks.value])
watch([orders,shipments],()=>{checks.value=reconcile(orders.value,shipments.value,checks.value)}, {deep:true})
...
const visibleShipments=computed(()=>shipments.value.filter(s=>allocations(s).some(a=>visible.value.some(o=>o.id===a.order))))
...
const selectedShipments=computed(()=>selected.value?shipments.value.filter(s=>allocations(s).some(a=>a.order===selected.value.id)):[])
...
function reset(){overviewSelection.value=null;orders.value=workspaceOrders();issues.value=seedIssues().filter(i=>i.id!=='EX-002');shipments.value=workspaceShipments();checks.value=reconcile(orders.value,shipments.value);synced.value=false;selected.value=null;detailOpen.value=false;modalOpen.value=false;query.value='';filter.value='open';toast('演示已恢复初始状态。')}
...
function performReview(c,input){
  ...
  try{
    ...
    checks.value=reconcile(orders.value,shipments.value,checks.value)
...
function performDispatch(input){try{if(input.allocations.some(a=>!visible.value.some(o=>o.id===a.order)))throw new Error('order_missing');const id=recordDispatch(orders.value,shipments.value,input);for(const a of input.allocations)addEvent(orders.value.find(o=>o.id===a.order),`登记业务报发 ${id}`,`Recorded dispatch ${id}`);checks.value=reconcile(orders.value,shipments.value,checks.value);toast(label('报发已登记；SAP 过账保持独立。','Dispatch recorded; SAP PGI remains separate.'))}catch(e){throw new Error(tr(actionErrors[e.message]||e.message))}}
function syncDemo(){if(role.value!=='buyer')return;if(applyDemoSnapshot(orders.value,shipments.value)){synced.value=true;for(const o of orders.value.filter(o=>o.po==='450051727'))addEvent(o,'接收后续 SAP 示例快照，关联交货单 DEMO-DLV-003','Received later demo SAP snapshot; linked DEMO-DLV-003');checks.value=reconcile(orders.value,shipments.value,checks.value);toast(label('示例过账已匹配，原报发记录保持不变。','Demo PGI matched. Original dispatch reports are unchanged.'))}}
function saveCollaborationDates(o,input){if(!visible.value.some(r=>r.id===o.id))return;if(!input.reason.trim()||!validDate(input.etd)||input.etd<TODAY||input.china&&!validDate(input.china))throw new Error(tr(label('请填写有效日期与变更原因。','Enter valid dates and a reason.')));const old=o.promisedEtd;o.promisedEtd=input.etd;o.chinaEta=input.china;o.dispatchReason=input.reason;addEvent(o,`发运承诺 ${old||'TBD'} → ${input.etd}；${input.reason}`,`Dispatch commitment ${old||'TBD'} → ${input.etd}; ${input.reason}`);toast(label('协作日期已更新。','Collaboration dates updated.'))}
...
<template>
  <div class="workspace" :class="{'overview-layout':page==='overview'}">
    ...
    <div class="main-shell">
      ...
      <main :class="{'overview-main':page==='overview','orders-main':page==='orders'}">
    ...
    <Dialog v-model="modalOpen" :options="{title:tr(modalTitle),size:modal==='help'?'xl':'lg'}"><template #body-content>
      ...
      <template v-else-if="modal==='shipment'"><p class="modal-description">{{tr(activeShipment?.id)}} · {{tr(activeShipment?.ref)}}</p><label class="textarea-label">{{tr("运输节点")}}<select v-model="shipmentStage"><option :value="0">{{tr({zh:'报发待核实',en:'Reported, unverified'})}}</option><option :value="2">{{tr("在途")}}</option><option :value="3">{{tr("已到港")}}</option></select></label><label class="textarea-label">{{tr("当前位置")}}<input v-model="shipmentLocation" :placeholder="tr(&quot;例如：已抵达墨尔本港，待清关&quot;)"/></label><label class="textarea-label">{{tr("运输预计到仓")}}<input type="date" v-model="shipmentEta" :min="TODAY"/></label><p class="muted">{{tr("此操作不更新 SAP 发货、收货数量或供应商原承诺。")}}</p></template>
