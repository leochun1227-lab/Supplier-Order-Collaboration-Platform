...
export function workspaceShipments(){
  return [...seedShipments().map(s=>({...s,sapPosted:true,sapDelivery:`DEMO-DLV-${s.id.slice(-3)}`,reportedAt:s.etd,pgiAt:s.etd,chassis:'DEMO-VAN',position:label('配件区','Parts compartment'),allocations:[{order:s.order,qty:s.qty,receivedQty:s.receivedQty}]})),
    {id:'SHP-DEMO-003',order:'450051727-00090',qty:78,receivedQty:0,mode:'海运',ref:'DEMO-CONTAINER-03',carrier:'Demo carrier',from:'宁波',to:'墨尔本仓',etd:'2026-09-04',reportedAt:'2026-09-04',eta:'',originalEta:'',stage:0,location:label('已报发，物流节点待核实','Reported; logistics unverified'),updated:'09/04 16:00',sapPosted:false,sapDelivery:'',pgiAt:'',chassis:'DEMO-VAN-03',position:label('车内配件区','Interior parts compartment'),allocations:[{order:'450051727-00090',qty:48,receivedQty:0},{order:'450051727-00100',qty:30,receivedQty:0}]}]
}
...
export function detectChecks(orders,shipments,today=TODAY){
  ...
  for(const o of orders){
    ...
    if(remainingQty(o)>0&&(!o.promisedEtd||o.promisedEtd<today))add(o,'delivery',`${o.promisedEtd||'TBD'}; ${remainingQty(o)} ${o.unit}`,'高')
...
export function reconcile(orders,shipments,previous=[],today=TODAY){
  ...
  const current=detected.map(c=>{const prior=old.get(c.id);return prior?{...prior,...c,owner:prior.owner,due:prior.due,note:prior.note,log:prior.log||[],status:prior.status!=='已解决'&&prior.fingerprint===c.fingerprint?prior.status:'待处理'}:{...c,log:[]}})
...
export function recordDispatch(orders,shipments,input,today=TODAY){
  if(!validDate(input.date)||input.date>today||!input.ref?.trim()||!input.allocations?.length||!['海运','空运'].includes(input.mode))throw new Error('dispatch_required')
...
export function businessSummary(orders,shipments,cases=[]){
  ...
  const risk=open.filter(o=>daysLate(o)>0||remainingQty(o)>0&&(!o.promisedEtd||o.promisedEtd<TODAY))
