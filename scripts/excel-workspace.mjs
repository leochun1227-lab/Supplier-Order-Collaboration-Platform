...
export function excelWorkspace(batch){
  const orders=[],shipments=[],issues=[],importedAt=new Date().toISOString().slice(0,10)
  for(const record of Object.values(batch.records)){
    ...
    const o={
      ...
      sourceAsOf:batch.sourceAsOf||'',sapAsOf:p?batch.sapAsOf||'':'',syncedAt:'',cooperationSource:{zh:'Excel 初始化／平台维护',en:'Excel initialization / platform'},
      batches:[],pending:null,comments:[],dispatchUpdates:[],history:[{who:'Excel',time:importedAt,text:{zh:'已导入原始台账，关联与状态待核对。',en:'Source ledger imported; associations and status require review.'}}],
    ...
    if(reported>0&&!cancelled){
      shipments.push({id:`IMP-${record.id}`,imported:true,aggregate:true,order:o.id,qty:reported,receivedQty:null,allocations:[{order:o.id,qty:reported,receivedQty:null}],
        ...
        delayStatus:'unknown',delayReason:'',nextAction:'',updates:[],sapPosted:false,sapDelivery:'',pgiAt:'',from:'',to:'',updated:importedAt})
    }
  }
  for(const [id,task] of Object.entries(batch.reviewTasks||{})){
    if(task.record||task.kind!=='source_link')continue
    const row=batch.sourceRows[task.evidence?.[0]]
    if(!row)throw new Error('missing_supplement_evidence')
    const f=row.fields,reference=[row.source.file,row.source.sheet,row.source.row].join(' / ')
    issues.push({id:`IMPORT-${id}`,order:`SOURCE-${row.id}`,kind:'source_link',category:'数据问题',
      title:{zh:'补充表记录待关联',en:'Supplement row requires linking'},
      detail:{zh:`${reference}；未建立唯一主表关联，不计为新订单。候选主表行：${(task.detail?.candidateMasterRows||[]).join(', ')||'无'}`,en:`${reference}; no unique master link; not counted as a new order. Candidate rows: ${(task.detail?.candidateMasterRows||[]).join(', ')||'none'}`},
      owner:text(f.buyer)||'—',priority:'中',due:importedAt,status:'待处理',note:'',
      sourceRecord:{po:text(f.po),item:text(f.line),part:text(f.material),soPart:text(f.material),importFields:f,importEvidence:[row.source]},
    })
  }
