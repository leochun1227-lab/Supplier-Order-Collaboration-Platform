...
The first page is the Overview dashboard; Orders is the second navigation item. Overview uses a compact title/date/filter toolbar directly above the KPI cards. It includes outstanding PO/line counts, in-transit and unshipped quantities and goods values, fulfillment flow, transport quantity/value toggle, production/after-sales categories, open-order age buckets, shipment ETAs, freight and supplier confirmation summaries. Supplier/category filters apply to overview metrics; chart drilldowns carry the exact selected order IDs into the order workbench and can be cleared there.

Overview now uses five compact KPI tiles (including received quantity/value), three adjacent summary panels and an ETA/task row at desktop widths. The fulfillment and transport charts use compact stacked bars; classification and supplier confirmation share one panel. The sidebar is narrower only while viewing Overview. The earliest three shipment ETAs appear on the dashboard, with the complete list available through All shipments. Metric definitions are expandable, and narrow screens reflow rather than shrink the text or clip data.

Fulfillment analysis replaces the duplicate reports page. It describes the historical records needed for supplier response, commitment reliability and transport analysis; no historical performance rates are claimed. Quantity reconciliation is in Data center, and Overview shows only the count of open data issues with a link to the appropriate work area.
